...
# significance
sigVals <- data.frame(t(bestmod_summary$P)[,-1])
names(sigVals) <- names(sigLabel) <- bestmod$names[-1]
sigLabel <- as.data.frame(bestmod$weights[[1]][,-1])  %>% mutate(across(1:4, \(x) ifelse(x>0, "+","-")))
...
sigLabel <- pivot_longer(sigLabel, cols = 1:4, names_to = "coef", values_to = "text")
