...
# fit a basic model with no biotic interactions or spatial autocorrelation.
occ_modelsb[["base"]] <- sjSDM(Y = occ_data_binb, env = linear(data = scale(env_matb), formula = ~alt+landcov+ghm), # scales env. covariates: land cover (forest/grass), altitude, ghm, dist to pa
                              se = TRUE, family=binomial("probit"), device = "cpu")
SPeigenb <- generateSpatialEV(env_matb[,c(1,2)])
occ_modelsb[["spatEV"]] <- sjSDM(Y = occ_data_binb, env = linear(data = scale(env_matb), formula = ~alt+landcov+ghm), # scales env. covariates: land cover (forest/grass), altitude, ghm, dist to pa
                                spatial = linear(SPeigenb, ~0+.),
                                se = TRUE, family=binomial("probit"), device = "cpu")
occ_modelsb[["spatlin"]] <- sjSDM(Y = occ_data_binb, env = linear(data = scale(env_matb), formula = ~alt+landcov+ghm), # scales env. covariates: land cover (forest/grass), altitude, ghm, dist to pa
