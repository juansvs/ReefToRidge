...
data_raw <- read_csv("Data/ct_records.csv",
  col_types = cols_only(deployment_id = "c",
                        common_name = "c",
                        timestamp = col_datetime("%Y-%m-%d %H:%M"))
)
# include only cameras with more than 12 weeks (~3 months)
cams_include <- summarise(data_raw, .by = deployment_id,
...
daytemp_sum <- summarise(temp_data_day, max_T = max(LST),
                         min_T = min(LST), mean_T = mean(LST),
                         sd_T = sd(LST), .by = site)
...
sp_order <- c("central_american_agouti", "oncilla", "baird's_tapir",
              "greater_grison", "white_lipped_peccary", "ocelot",
              "white_nosed_coati", "guan_black", "tinamou_great",
              "margay", "puma", "spotted_paca", "tayra",
              "northern_tamandua", "coyote", "crab_eating_raccoon",
              "jaguar", "tinamou_little", "collared_peccary",
              "guan_crested", "striped_hog_nosed_skunk",
              "central_american_red_brocket", "common_opossum",
              "jaguarundi", "cottontail_dices", "opossum_four_eyed",
              "nine_banded_armadillo", "great_curassow")
...
y_df <- filter(DAT, !grepl("uid", common_name)) %>%
  ...
  filter(max(smplwk) > 4, .by = site) %>% # keep only stations working 4+ weeks
...
covs_df <- distinct(y_df, site) %>%
  ...
  left_join(daytemp_sum) %>%
  select(site, easting, northing, ghm1, lc2, alt1,
...
moddata <- list(y = mody,
                occ.covs = covs_df[, c("forest", "alt", "ghm", "mean_T",
...
# initial values
inits_list <- list(beta.comm = 0, alpha.comm = 0, # Community-level occurrence (beta.comm) and detection (alpha.comm) regression coefficients
  ...
  z = apply(mody, c(1, 2), max, na.rm = TRUE)
)


#### Run models ####
# Null model, intercept only
sfmod_null <- sfMsPGOcc(
  occ.formula = ~1,
  det.formula = ~1,
  data = moddata,
  inits = inits_list,
...
summary(sfmod_null)
waicOcc(sfmod_null)
# Null model wAIC: 9056.26 The convergence for the spatial covariance terms is
...
# although there are still issues with the spatial covariance
# estimates (Rhat = 1.3,1.0,1.1, 3.1).
...
# 8026.6, much lower than the non-updated model
...
sfmod_full <- update(sfmod_null,
  occ.formula = ~treecov + pasture + ghm + scale(mean_T),
...
sfmodalt <- update(sfmod_full, occ.formula = ~scale(alt))
sfmodalt2 <- update(sfmod_full, occ.formula = ~scale(alt) + I(scale(alt)^2))
sfmodalt2ghm <- update(sfmod_full,
                       occ.formula = ~scale(alt) + I(scale(alt)^2) + scale(ghm))
# summary: better Rhat values than the full model.
sfmodaltghm <- update(sfmod_full, occ.formula = ~scale(alt) + scale(ghm))
sfmodaltlc <- update(sfmod_full, occ.formula = ~forest + scale(alt))
sfmodalt2lc <- update(sfmod_full,
  occ.formula = ~forest + scale(alt) + I(scale(alt)^2)
)
sfmodtempalt2lc <- update(sfmod_full,
  occ.formula = ~scale(mean_T) + I(scale(alt)^2) + forest
)
sfmodtemplc <- update(sfmod_full, occ.formula = ~scale(mean_T) + forest)


# Compare models based on wAIC
waicOcc(sfmod_full)    # waic = 8062 -> 7995.03 -> 8986.25 w/ temp
...
lf_mod_full <- lfMsPGOcc(
  occ.formula = ~forest + scale(alt) + I(scale(alt)^2) + scale(ghm),
  det.formula = ~1,
  data = moddata,
  inits = inits_list,
...
waicOcc(update(lf_mod_full, occ.formula = ~1)) # 8120
waicOcc(update(lf_mod_full, occ.formula = ~forest + scale(alt))) # 8075
waicOcc(lf_mod_full) # 8026 vs. 7995 for the spatial full model.
...
# diagnostics
ppc_out <- ppcOcc(sfmod_full, fit.stat = "freeman-tukey", group = 1)
summary(ppc_out)
# The full model's Bayesian p value is 0.238, so not terrible,
# but rather low (0.5 is good, <0.1 or >0.9 is poor). The fit is
# terrible for agoutis (p=0), and p<0.1 for collared peccaries,
# common opossums, rabbits, curassows, pumas, pacas, and coatis.
...
modpreds <- predict(sfmod_full,
  data.frame(int = 1, alt = scale(moddata$occ.covs$alt),
             alt2 = scale(moddata$occ.covs$alt)^2,
             ghm = scale(moddata$occ.covs$ghm)),
  moddata$coords, type = "occupancy"
)
predrich <- apply(modpreds$z.0.samples, c(1, 3), sum) |>
  apply(2, quantile, p = c(0.25, 0.5, 0.75))
rownames(predrich) <- c("lq", "med", "uq")
predrich <- t(predrich)

data.frame(site = rownames(comm), nobs = specnumber(comm)) %>%
  right_join(covs_df) %>%
  cbind(predrich) %>%
  ggplot(aes(x = nobs)) +
  geom_abline(slope = 1) +
...
coords.0 <- matrix(colMeans(moddata$coords),
                   ncol = 2, nrow = nrow(X.0), byrow = TRUE)
# prediction covars. vary only the altitude, leave other covariates at 0
X.0 <- data.frame(intercept = 1, forest = rep(c(0, 1), each = 3),
                  alt = altpredvars, alt2 = altpredvars^2,
                  ghm = rep(c(-2, 2), each = 3))

modpredsocc <- predict(sfmod_full, X.0 = X.0, coords.0 = coords.0)[['psi.0.samples']]
dim(modpredsocc)

# Plot of occupancy vs alt (L,M,H) for example spp
preddb <- data.frame(
  expand.grid(1:1200,
              sp = sfmod_full$sp.names, alt = altpredvars,
              ghm = c(-2, 2)),
  psi = as.matrix(modpredsocc)
)

# data.frame(expand.grid(sp = sfmodalt2$sp.names, alt=factor(newalts))) %>%
# c("white_nosed_coati", "collared_peccary", "tinamou_great",
# "ocelot", "jaguar", "baird's_tapir", "spotted_paca")
# keep only a subset of species to represent the different patterns
filter(preddb, sp %in% c("tinamou_great", "jaguar", "ocelot",
  ...
  stat_summary(aes(color = factor(ghm)), fun = "median",
               fun.min = \(x) quantile(x, p = 0.25),
               fun.max = \(x) quantile(x, p = 0.75),
...
beetjsdmfull <- lfJSDM(formula = ~forest + scale(alt) + I(scale(alt)^2) +
                         scale(ghm),
                       data = beet_data,
                       n.samples = 2000, n.thin = 2, n.chains = 3,
                       n.factors = 4, n.burn = 1000)
...
beetjsdmalt2ghm <- update(beetjsdmnull,
                          formula = ~scale(alt) + I(scale(alt)^2) + scale(ghm))
beetjsdmalt2lc <- update(beetjsdmnull,
                         formula = ~forest + scale(alt) + I(scale(alt)^2))
...
# The full has the lowest wAIC, but it is comparable to the
# mode model with alt, alt^2, and ghm, and the one with alt and lc.
summary(beetjsdmalt2ghm, "community")
summary(beetjsdmaltlc, "community")

### Models with spatial covars
sf_beet_jsdm_null <- sfJSDM(~1,
  data = beet_data,
...
sf_beet_jsdm_full <- update(sf_beet_jsdm_null,
                            formula = ~forest + scale(alt) +
                              I(scale(alt)^2) + scale(ghm))
sapply(list(sf_beet_jsdm_full, sf_beet_jsdm_null), waicOcc)
# Here too the full model performs better

sf_beet_jsdm_alt2 <- update(sf_beet_jsdm_full,
                            formula = ~scale(alt) + I(scale(alt)^2))
sf_beet_jsdm_alt <- update(sf_beet_jsdm_full, formula = ~scale(alt))
sf_beet_jsdm_altghm <- update(sf_beet_jsdm_full,
                              formula = ~scale(alt) + scale(ghm))
sf_beet_jsdm_alt2ghm <- update(sf_beet_jsdm_full,
                               formula = ~scale(alt) + I(scale(alt)^2)+scale(ghm))
sf_beet_jsdm_altlc <- update(sf_beet_jsdm_full, formula = ~forest+scale(alt))
sf_beet_jsdm_alt2lc <- update(sf_beet_jsdm_full,
                              formula = ~forest + scale(alt) + I(scale(alt)^2))
sf_beet_jsdm_altlcghm <- update(sf_beet_jsdm_full,
                                formula = ~forest + scale(alt) + scale(ghm))

# AIC ranking
sapply(list(sf_beet_jsdm_full, sf_beet_jsdm_altghm,
            sf_beet_jsdm_altlcghm, sf_beet_jsdm_alt2ghm,
            sf_beet_jsdm_alt2lc), waicOcc)
...
#####--- Predicted occupancy ####
cent_alt <- attr(scale(beet_data$covs[,"alt"]), "scaled:center")
scal_alt <- attr(scale(beet_data$covs[,"alt"]), "scaled:scale")
...
# prediction covars.
X.0 <- data.frame(intercept = 1, forest = 1,
                  alt = altpredvars, alt2 = altpredvars^2,
                  ghm = rep(newghm, each = length(newalts)))
X.0 <- as.matrix(X.0)
# get mean coordinates
coords.0 <- matrix(colMeans(beet_data$coords),
                   ncol = 2, nrow = nrow(X.0), byrow = TRUE) 
...
# data.frame(expand.grid(sp = sfmodalt2$sp.names, alt=factor(newalts))) %>% 
# keep only a subset of species to represent the different patterns
filter(preddb, sp %in% c("Onthophagus_acuminatus", "Coprophanaeus_telamon",
                         "Coprophanaeus_pecki", "Canthon_angustatus",
                         "Onthophagus_praecellens","Onthophagus_haematopus")) %>%
  ggplot(aes(factor(alt), psi, fill = factor(ghm))) +
  geom_violin(scale = "width", show.legend = FALSE) +
  stat_summary(aes(color = factor(ghm)),
               fun = "median",
               fun.min = \(x) quantile(x, p = 0.25),
               fun.max = \(x) quantile(x, p = 0.75),
               position = position_dodge(0.9), show.legend = FALSE) +
